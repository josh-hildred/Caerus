...
//      LOG(ERROR) << "txn " << txn->txn_id() << " has readers " << txn->readers(i);
  ...
  if (reader) {
    ...
    if (mode_ == 0) {
      ...
      for (auto remote_writer : remote_replica_writers_) {
        ...
        //  LOG(ERROR) << "sending message to " <<  configuration_->LookupMachineID(mds, configuration_->local_replica_id()) << " on channel " << destination_channel;
