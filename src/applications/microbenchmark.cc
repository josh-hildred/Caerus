...
#else
    uint64_t hotkey_order2;
    do {
        hotkey_order2 = (rand() % (hot_records / replica_size)) * replica_size + replica1;
    } while (hotkey_order1 == hotkey_order2);
#endif
...
TxnProto* Microbenchmark::MicroTxnMRMP(int64 txn_id, uint64 part1, uint64 part2, uint32 replica1, uint32 replica2) {
  ...
  uint64 hotkey_order2 = (rand() % (hot_records/replica_size)) * replica_size + replica2;
#else
    uint64 hotkey_order2 = (rand() % (hot_records/replica_size)) * replica_size + replica1;
#endif
    uint64 hotkey1 = part1 + nparts * hotkey_order1;
  uint64 hotkey2 = part2 + nparts * hotkey_order2;
...
LOG(ERROR) <<local_replica_<< ":*********In Execute:  handle remaster txn: "<<txn->txn_id();
  ...
  double execution_start = GetTime();
    ...
    char read_modifier = 1;
    //if(txn->read_set_size() == 1){
        {
            KeyEntry key_entry = txn->read_write_set(0);
            Record *val = storage->ReadObject(key_entry.key());
            read_modifier = (val->value)[0];
        }
    //}
...
LOG(ERROR) <<local_replica_<< ":*********In Execute:  Generate a remaster  txn, on record: "<<key_entry.key()<<"  txn id:"<<txn->txn_id();
    ...
    for (int j = 0; j < 8; j++) {
      if ((val->value)[j] + 1 > 'z') {
        ...
        //int new_val =  'a' + ((read_modifier + ((val->value)[j] * (txn->txn_id() % 26))) % 26);
        int new_val =  'a' + (((txn->txn_id()/val->value[j]) * ((val->value)[j] + (txn->txn_id() % 26))) % 26);
        ;
        (val->value)[j] = new_val;
    }
  }
