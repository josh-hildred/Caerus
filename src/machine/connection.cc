...
//    pthread_create(&thread_, nullptr, RunMultiplexer, reinterpret_cast<void*>(this));
...
uint64_t ConnectionMultiplexer::ChannelSize(const std::string &channel) {
    CHECK(channel_results_.Count(channel) > 0);
    return channel_results_.Lookup(channel)->Size();
}
...
void ConnectionMultiplexer::Run() {
  ...
  bool got_request = false;
    ...
    uint64_t in_throughput = 0;
    uint64_t out_throughput = 0;
    double time = GetTime();
#endif
  ...
  while (!deconstructor_invoked_) {
      ...
      if(GetTime() - time > 20)
      {
          ...
          << in_throughput/(GetTime() - time) <<"\noutgoing message throughput: " << out_throughput/(GetTime() - time)
          << "----------------------------------------------------------------";
          in_throughput = 0;
          out_throughput = 0;
          time = GetTime();
...
//LOG(ERROR) << local_node_id_ << ":ConnectionMultiplexer::Run(), receive a meesage1, channel:"<<message.destination_channel();   
    ...
    if (got_request == true) {
      ...
      if (message.destination_node() == local_node_id_) {
        ...
        //LOG(ERROR) << message.destination_channel();
