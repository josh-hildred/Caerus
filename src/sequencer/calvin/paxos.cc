...
void Paxos::RunLeader() {
  ...
  while (go_) {
    ...
    {
      Lock l(&mutex_);
      tput += sequence_.batch_ids_size() * 100;
...
//LOG(ERROR) <<this_machine_id_<< ":In paxos log:  send PAXOS_BATCH_ORDER: "<<version<<"  to node:"<<i;
    ...
    for (uint32 i = 1; i < participants_.size(); i++) {
      ...
      if(GetTime() - start > PRINT_INTERVAL) {
          LOG(ERROR) << "MACHINE " << configuration_->local_node_id() << ": Paxos leader sequence message throughput: " << (tput/(GetTime() - start));
          tput = 0;
          start = GetTime();
      }
