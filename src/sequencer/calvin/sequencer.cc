...
map<uint64, double> scheduler_recv;
std::atomic<uint64> latency_counter;
vector<double> measured_latency;
vector<double> measured_latency_start_sch;
vector<double> measured_latency_sch_done;
map<uint64_t, bool> mr_txn;
vector<int> mr_flag;
#endif
...
Sequencer::Sequencer(ClusterConfig* conf, ConnectionMultiplexer* connection, Client* client, Paxos* paxos, uint32 max_batch_size)
          : epoch_duration_(EPOCH_DURATION), configuration_(conf), connection_(connection),
...
void Sequencer::RunReader() {
    ...
    uint64_t PaxosTPut = 0;
    double leader_start = GetTime();

LOG(ERROR) << "In sequencer:  Starting sequencer reader.";
    LOG(ERROR) << "number of replicas: " << configuration_->replicas_size();
  // Set up batch messages for each system node.
...
//LOG(ERROR) << configuration_->local_node_id()<< ":In sequencer reader:  recevie TXN_BATCH message:"<<message.batch_number();
        ...
        tput += message.data_size();
        if(GetTime() - start > 5)
        {
            LOG(ERROR) << "Machine: " << configuration_->local_node_id() << " Replica: " << local_replica
            << "Sequencer throughput: " << tput/(GetTime() - start);
            tput = 0;
            start = GetTime();
        }
